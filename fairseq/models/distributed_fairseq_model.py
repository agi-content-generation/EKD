...
def DistributedFairseqModel(args, model, process_group=None):
    ...
    if args.distributed_wrapper == "DDP" and args.ddp_backend == "c10d":
        ...
        print('ddp_class_type:', type(ddp_class))
        

        try:
            sig = inspect.signature(ddp_class.__init__)  # 使用 __init__ 方法
            if "check_reduction" in sig.parameters:
                init_kwargs["check_reduction"] = True
            if "find_unused_parameters" in sig.parameters:
                init_kwargs["find_unused_parameters"] = args.find_unused_parameters
        except ValueError as e:
            print(f"ValueError: {e}")  # 打印错误信息，帮助调试


        # # if "check_reduction" in inspect.getargspec(ddp_class)[0]:  # inspect.signature()
        # if "check_reduction" in inspect.signature(ddp_class)[0]:
        #     init_kwargs["check_reduction"] = True
        # # if "find_unused_parameters" in inspect.getargspec(ddp_class)[0]: # inspect.signature()
